...
*************************
					...
					<!-- -->
					<xsl:for-each select="mods:targetAudience">
...
-->
	<xsl:template name="subject">
		...
		<xsl:comment>
			...
			<xsl:if test="$subjectType='Complex'">
			<xsl:text> --   </xsl:text>		
			<xsl:for-each select="*">
...
-->
	<xsl:template name="geographic">
		...
		<Geographic xmlns="http://www.loc.gov/mads/rdf/v1#">
			<xsl:choose>
				<xsl:when test="$subType='GeographicCode' or $subType='Geographic'">
					<!-- 
                         its a it's a geopraphicCode or Geographic 
-->
					<rdfs:label>
						<xsl:value-of select="."/>
					</rdfs:label>
					<elementList rdf:parseType="Collection">
						<xsl:element name="{concat($subType,'Element')}">
							<elementValue>
								<xsl:value-of select="."/>
							</elementValue>
						</xsl:element>
...
-->
					...
					<elementList rdf:parseType="Collection">
						<xsl:for-each select="*">
							<xsl:element name="{concat( upper-case( substring(local-name() , 1 , 1) ) , substring(local-name() , 2 ), 'Element' )}">
								<elementValue>
									<xsl:value-of select="."/>
								</elementValue>
							</xsl:element>
	...
	<xsl:template name="nameTitle">
		...
		<NameTitle xmlns="http://www.loc.gov/mads/rdf/v1#">
			<rdfs:label>
				...
				<xsl:text> -- </xsl:text>
				<xsl:call-template name="titleString" use-when="mods:titleInfo[(@type='uniform')]"/>
			</rdfs:label>
			<componentList rdf:parseType="Collection">
				<xsl:value-of select="$newline"/>
				<!-- 
