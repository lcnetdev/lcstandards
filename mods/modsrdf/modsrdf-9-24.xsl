...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:modsrdf="http://www.loc.gov/mods/rdf/v1#" xmlns:madsrdf="http://www.loc.gov/mads/rdf/v1#" xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" xmlns:identifier="http://id.loc.gov/vocabulary/identifier/" xmlns:relator="http://id.loc.gov/vocabulary/relator/" xmlns:note="http://id.loc.gov/vocabulary/note/" xmlns:abstract="http://id.loc.gov/vocabulary/abstract/" xmlns:access="http://id.loc.gov/vocabulary/access/" xmlns:class="http://id.loc.gov/vocabulary/class/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:ri="http://id.loc.gov/ontologies/RecordInfo#">
	...
	<xsl:include href="identifier-types.xsl"/>
	<!-- 
*******************************************************
                      Global Variables 
******************************************************
...
-->
	<!--
List of identifier typess.  Used in identifier template.
...
5. Process identifiers (all except type="modsRDFIdentifier" )
6. Process subjects
...
This requires that an identifier of type 'modsRDFIdentifier' has been added to the MODS record, explicitly for this purpose. For the above example the following would be added to the MODS record:

	<identifier type="modsRDFIdentifier">MODS12345</identifier>
-->
		<xsl:comment>
*******************************************************
   Top level element
*******************************************************
		...
		<xsl:variable name="modsRDFIdentifier">
			<xsl:choose>
				<xsl:when test="mods:identifier[@type='modsRDFIdentifier']">
					<xsl:value-of select="mods:identifier[@type='modsRDFIdentifier']"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:element name="modsrdf:ModsResource">
			<xsl:attribute name="rdf:about"><xsl:value-of select="$modsRDFIdentifier"/></xsl:attribute>
...
**************************************************************************************
	-->
			<xsl:if test="mods:identifier[not(@type) or @type!='modsRDFIdentifier']">
				<xsl:comment>
...
***************************
				...
				<xsl:for-each select="mods:identifier">
					<xsl:if test="not(@type) or @type!='modsRDFIdentifier'">
...
*******genre
	...
	<!--		
********************Template for MODS identifer element.  (Other than type="modsRDFIdentifier".)
...
<!-- attribute -->			
			...
			<xsl:choose>
			<xsl:when test="$value castable as xsd:date or $value castable as xsd:gYear">
			...
			<!-- value -->			
						<xsl:choose>
			<xsl:when test="$value castable as xsd:date">
			<xsl:value-of select="xsd:date($value)"/>
			</xsl:when>
			<xsl:otherwise>
			<xsl:value-of select="$value"/>
			</xsl:otherwise>
